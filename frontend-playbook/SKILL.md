...
- **Required techniques (≥4 must appear in the signature moment):**
  ...
  - [ ] `ScrollTrigger` `scrub` for scroll-linked parallax/progress — **without `pin: true`** (scroll must always advance the page; see Motion Taste Principle "No scroll-jacking pin traps"). `toggleActions` once-reveal is the fallback when scrub isn't needed.
...
- **When to escalate to Three.js — see `three-d.md` (the 3D signature manual) + `motion-templates.md` §10 (reference code):** CSS/SVG handles 90% of signature moments. Escalate to real 3D when ANY of: (a) base state needs real perspective/fog/lighting that CSS `perspective` cannot fake; (b) cursor interaction is a **raycast onto a 3D surface** displacing nearby geometry along the surface normal (the "igloo.inc" signature — cursor parts the assembled object, nearby pieces lift toward camera); (c) the signature is physical objects in space (polaroids floating, product cards on a wall, architectural slabs); (d) the material IS the subject (concrete, glass, steel, water, fabric — PBR beats 2D). Stack: `three` + `@react-three/fiber`. Use per-frame `lerp` (k≈0.18 toward target, k≈0.06 spring-back home) — NOT gsap.elasticOut, which is for one-shot tweens, not continuous cursor tracking. DO NOT escalate for 2D hovers, scroll-triggered tweens, or anything achievable with templates 1–9.
  - **MANDATORY domain check before coding:** `three-d.md` §3 is a domain→3D-signature decision matrix (7 categories A–G). State your category letter + matrix row + one-sentence subject-embodiment test BEFORE writing any 3D code. A flat rectangle grid on an architecture site (or any non-discrete-object domain) = FAIL the taste gate — it is the #1 LLM-default collapse the matrix exists to prevent. `motion-templates.md` §10 is reference code for category B only; confirm your domain is actually a B domain before reaching for it.
